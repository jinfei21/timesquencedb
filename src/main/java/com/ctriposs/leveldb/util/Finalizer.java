...
public class Finalizer<T> {
	...
	public Finalizer(int threads){
		Preconditions.checkArgument(threads >= 1, "threads must be at last 1");
		ThreadFactory threadFactory = new ThreadFactoryBuilder()
										.setNameFormat("Finalizer-%d")
										.setDaemon(true)
										.build();
		this.stop = false;
		this.executor = Executors.newFixedThreadPool(threads,threadFactory);
		for(int i=0;i<threads;i++){
			this.executor.submit(new FinalizerProcessor());
		}
	}
    ...
    private class FinalizerProcessor implements Runnable{
					 ...
					 try{
		                ...
		                if (rescheduleAndReturn) {
		                    synchronized (Finalizer.this) {
		                        if (!stop) {
		                            executor.submit(new FinalizerProcessor());
		                        }
		                    }
		                    return;
		                }
